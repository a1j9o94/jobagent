...
from app.db import get_session, health_check as db_health_check, engine # Added engine import
...
from app.models import Profile, UserPreference, Application, ApplicationStatus, Role # Added Role
...
async def health_check_endpoint(): # Renamed to avoid conflict with imported health_check functions
    ...
    if health_status["status"] == "critical":
        return Response(
            content=str(health_status), #fastapi expects content as str or bytes
...
async def handle_whatsapp_reply(request: Request, session: Session = Depends(get_session)): # Added session dependency
...
• Or answer any pending questions
            ...
            from app.tasks import task_send_daily_report # Local import to avoid circular dependency issues at startup
            ...
            send_whatsapp_message("📊Generating your daily report, it will arrive shortly!", from_number)
...
async def get_applications(
    ...
    query = select(Application).join(Role) # Added join to access Role attributes easily
    ...
    for app_db in applications_db:
        app_data = {
            "id": app_db.id,
            "role_title": app_db.role.title if app_db.role else "N/A", # Handle if role is somehow None
            "company_name": app_db.role.company.name if app_db.role and app_db.role.company else "N/A",
            ...
            "submitted_at": app_db.submitted_at.isoformat() if app_db.submitted_at else None
        }
