...
from sqlmodel import Session as SQLModelSession # Renamed to avoid conflict with celery Session
...
def task_send_daily_report(profile_id: int = 1): # Added profile_id as arg with default
    ...
    try:
        ...
        if success:
            ...
            return {"status": "error", "message": "Failed to send report", "profile_id": profile_id}
...
def task_process_new_roles(profile_id: int = 1): # Added profile_id as arg with default
    ...
    try:
        ...
        with get_session_context() as session:
            ...
            new_roles_stmt = select(Role).where(Role.status == RoleStatus.SOURCED).limit(10)
            ...
            logger.info(f"Queued {processed_count} roles for processing for profile {profile_id}")
            return {"status": "success", "processed": processed_count, "profile_id": profile_id}
